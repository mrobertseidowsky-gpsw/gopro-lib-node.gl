...
int ngli_program_init(struct program *s, struct ngl_ctx *ctx, const char *vertex, const char *fragment, const char *compute)
{
    ...
    while ((entry = ngli_hmap_next(s->attributes, entry))) {
        struct program_variable_info *info = entry->data;
        LOG(ERROR, "info->location=%d # %s", info->location, entry->key);
        info->location = ngli_glGetAttribLocation(gl, s->id, entry->key);
        LOG(ERROR, "info->location=%d #2 %s", info->location, entry->key);
    }
